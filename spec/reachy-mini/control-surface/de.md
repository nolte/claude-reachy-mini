...
> 🔗 **Tiefen-Referenzen.** Per-Joint-URDF-Limits, Stewart-Asymmetrie-Muster, kanonische Posen verbatim aus dem SDK-Source, T1–T8-Live-Verifikation, Drei-Schichten-Validitäts-Modell (IK-Polytop ⊋ URDF-Mechanik ⊋ Pollen-Nominal), Motor-Modi-Namen mit Placo-Constraint, Backend-Health-Diagnose und Recovery-Triage liegen in [`reachy-mini/motor-positions`](../motor-positions/de.md). Diese Spec hier nennt die *Operationsbereiche*; motor-positions liefert die *exakten Werte* und ihre Verifikation am echten Gerät.
...
- **Kollisionen** — **belegt 2026-05-12 durch realen Selbstkollisions-Vorfall:** die analytische IK akzeptiert Pose-Targets bis ±π pro Stewart-Joint (Software-Default aus `kinematics_data.json`) und macht keinen Self-Collision-Check (`engine: AnalyticalKinematics, collision check: false` aus `GET /api/kinematics/info`). Pose-Targets jenseits der Pollen-Nominal-Operations-Range (±40° Pitch/Roll, ±60° Head-Yaw) sind **IK-mathematisch lösbar, aber NICHT mechanisch sicher** — sie haben am realen Reachy Wireless zu Kopf-gegen-Körper-Kollision geführt. Verbindlich für jede Live-Bewegung ist die **innerste** der drei Validitäts-Schichten (Pollen-Nominal-Range); IK-Polytop und URDF-Mechanik sind nicht ausreichend. Details + Recovery-Triage in [`reachy-mini/motor-positions`](../motor-positions/de.md) Schicht 2 §"Drei Schichten der Validität" und Schicht 4 §"Phase-B-Live-Vorfall 2026-05-12"
- **Pitch-Bleed bei Roll und Heave-up** — **live verifiziert 2026-05-13:** die Stewart-Plattform-Geometrie koppelt reine Roll-Befehle und reine Heave-up-Befehle in Pitch. Quantitativ: `roll = +25°` zieht **−3.8°** Pitch nach unten; `z = +15 mm` zieht **+2.4°** Pitch nach vorne. Pure Pitch, pure Yaw und pure Heave-down (negativ z) zeigen keinen messbaren Bleed. Eine Bewegung, die isolierten Roll oder Heave braucht, muss die Pitch-Komponente in der Ziel-Pose explizit kompensieren — das ist keine Kalibrierungs-Drift, sondern systematische Plattform-Kopplung. Quelle: [`reachy-mini/motor-positions`](../motor-positions/de.md) Schicht 2 §"T1–T8 Live-Verifikation"
...
- **MUSS [MUST]** einen Notstopp-Pfad ansprechbar haben, der jede laufende Bewegung beendet und in eine Ruhepose fährt. Kanonische Ruhepose: `INIT_HEAD_POSE = np.eye(4)` (4×4-Identitäts-Matrix, Kopf zentriert) und `INIT_ANTENNAS_JOINT_POSITIONS = [-0.1745, +0.1745]` rad = `[-10°, +10°]` (verifiziert in [`reachy_mini.py`](https://github.com/pollen-robotics/reachy_mini/blob/main/src/reachy_mini/reachy_mini.py)). Alternativ steht die `SLEEP_HEAD_POSE` zur Verfügung (pitch **+24.4°** in xyz-Euler, Kopf nach vorne-unten geneigt; live verifiziert 2026-05-13 mit Real-Pitch ≈ +26°), die `goto_sleep()` ansteuert. Exakte Matrizen und IK-Joint-Lösungen siehe [`reachy-mini/motor-positions`](../motor-positions/de.md) Schicht 3
