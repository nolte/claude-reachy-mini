...
| Daemon-Stale-State nach mehrfachen App-Stop/Start-Cycles | App startet mit `ConnectionError: Could not connect to daemon on localhost. Is the Reachy Mini daemon running?` aus `reachy_mini.reachy_mini._initialize_client`, **obwohl** `systemctl is-active reachy-mini-daemon.service` weiterhin `active` meldet; der vorhin lauschende Port 6053 (App-eigener ESPHome-Server, falls vorhanden) listet nicht mehr und `_esphomelib._tcp` wird im mDNS nicht mehr beworben | Hardware-Daemon-Restart, siehe Recovery-Tabelle, Zeile „Daemon-Stale-State-Recovery" |
| Sanity-Check scheitert am fehlenden GStreamer-WebRTC-Plugin (direct-mode) | `RuntimeError: Failed to create webrtcsrc element. Is the GStreamer webrtc rust plugin installed?` direkt im `ReachyMini.__init__`, bevor irgendeine App-Logik läuft; gilt nur im **direct-mode**, nicht im daemon-hosting-Modus, weil dort der Daemon die GStreamer-Pipeline aufbaut | nicht App-bezogen — Verify-Basics-First-Heuristik § Setup-Voraussetzungen prüfen; Fallback auf eine **daemon-hosted Sanity-App** (eigenes minimales App-Skelett mit `wake_up()` + `goto_sleep()`), oder `gst-plugin-webrtc-rust` im erreichbaren GStreamer-Plugin-Pfad nachinstallieren |
...
- **MUSS [MUST]** beim direct-mode-Sanity-Check beachtet werden, dass `ReachyMini()` selbst die GStreamer/WebRTC-Pipeline aufbaut und `gst-plugin-webrtc-rust` im erreichbaren GStreamer-Plugin-Pfad voraussetzt; im **daemon-hosting**-Modus übernimmt der Daemon diese Initialisierung. Wenn der Sanity-Check mit `RuntimeError: Failed to create webrtcsrc element` stirbt, ist das **kein App-Bug**, sondern eine Setup-Lücke der direct-mode-Umgebung — die Triage-Klasse ist `webrtc-plugin-missing` aus dem Common-Issues-Katalog, nicht eine App-spezifische Klasse. In diesem Fall ist die Sanity-Probe nicht aussagekräftig; als Alternative ein eigenes minimales App-Skelett (Wake-Up + Sleep) im daemon-hosting-Modus anstoßen
- **SOLLTE [SHOULD]** der Lauf-Modus für den Sanity-Check bewusst gewählt werden: bei einer App, die produktiv im daemon-hosting läuft, ist eine daemon-hosted Sanity-App aussagekräftiger als ein direct-mode-Skript; bei einer App, die per `pytest` entwickelt wird, ist der direct-mode der natürliche Match
...
| Daemon-Stale-State-Recovery (nach mehrfachen App-Stop/Start-Cycles) | Wireless / Lite | wie Daemon-Restart, aber als gezielte Reaktion auf Common-Issues-Klasse `daemon-stale-state` — nicht als Routine | räumt den intern verklemmten WebSocket-Connection-Manager des Hardware-Daemons auf; HA reconnected automatisch via mDNS, sobald Port 6053 (App-eigener ESPHome-Server, falls vorhanden) wieder LISTEN steht |
...
- **SOLLTE [SHOULD]** bei aktiven Diagnose-Sessions, die mehrere App-Stop/Start-Cycles in kurzer Folge auslösen (Hot-Patch-Iteration, mehrere `reachy-mini-start`-Aufrufe), zwischen den Cycles ein Cooldown von etwa 30 s eingehalten werden, um Daemon-Stale-State zu vermeiden; alternativ den Daemon-Stale-State-Recovery-Befehl griffbereit halten
- **SOLLTE [SHOULD]** bei einem Daemon-Stale-State-Verdacht zuerst `ss -tln | grep ":6053"` (Wireless) oder ein vergleichbarer Listen-Check geprüft werden — schweigt der Port trotz `systemctl is-active=active`, ist die Klasse `daemon-stale-state` bestätigt
