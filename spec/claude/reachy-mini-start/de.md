...
### Lifecycle
- **MUSS** Pre-Flight-Checks vor jeder Zustandsänderung ausführen: Daemon-Erreichbarkeit, Installed-Apps-Listing, aktueller App-Lock-State
...
- **MUSS** nach dem Start via `GET /api/apps/current-app-status` (oder dem Äquivalent) verifizieren, dass der Daemon die angeforderte App als `running` meldet; ein 200 OK auf den Start-Call allein ist **nicht** ausreichend
...
- **DARF** den Pollen-Daemon unter keinen Umständen restarten, neuladen oder rekonfigurieren
...
- [ ] Der Skill restartet niemals den Pollen-Daemon
...
## Offene Fragen
- Welcher genaue REST-Endpoint startet eine installierte App — `POST /api/apps/start-app/<name>` mit dem Entry-Point-Namen oder eine Body-Form-Variante? Beim ersten Hardware-Kontakt verifizieren und im Skill-Body fixieren
- Liefert der `current-app-status`-Endpoint des Daemons synchron `running` zurück nach dem Start-Call, oder gibt es ein Settling-Fenster, in dem das Feld leer ist? Falls Settling-Fenster: Default-Poll-Budget definieren (Vorschlag: 5 s, alle 0,5 s)
- Bei Lite: ist die REST-Surface des Daemons identisch zu Wireless, oder hat der Host-PC-Daemon einen anderen Pfad-Prefix? Beim ersten Lite-Hardware-Kontakt verifizieren
